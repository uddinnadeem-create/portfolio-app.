...
st.set_page_config(page_title="Live Portfolio (Equities + Options)", layout="wide")

# ---------- Config ----------
REFRESH_SECONDS = int(os.getenv("REFRESH_SECONDS", "60"))
DEFAULT_TZ = os.getenv("APP_TIMEZONE", "Asia/Dubai")

st.title("📈 Live Portfolio Dashboard — Equities + Options")
...
if source_choice == "Google Sheets (CSV URLs)":
    ...
    st.sidebar.code("Equities: Ticker,Shares,AvgBuy\nOptions: Underlying,Expiry,Strike,C/P,Side,Qty,PremiumOpen,PremiumCurrent", language="text")
...
def fetch_last_prices(symbols):
    ...
    data = yf.download(uniq, period="1d", interval="1m", auto_adjust=False, prepost=False, threads=True, progress=False)
    last_prices = {}
    ...
    for yfs in uniq:
        if yfs not in last_prices:
            try:
                info = yf.Ticker(yfs).fast_info
                px = float(info.get("last_price", np.nan))
                if np.isfinite(px):
                    last_prices[yfs] = px
            except Exception:
                pass
...
prices = fetch_last_prices(eq["Symbol"].tolist())
...
st.dataframe(eq[["Ticker","Shares","AvgBuy","Price","Value","Cost","P/L","P/L %"]]
...
def bench_ret(ticker):
    try:
        dfb = yf.download(ticker, period="ytd", interval="1d", progress=False)
